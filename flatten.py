#!/usr/bin/env python3
"""
Flatten the wget mirror of starlinglab.org/anita/ into a simple static bundle.
...
# ---------------------------------------------------------------------------
# Mapping: disk path (relative to SRC) → destination path (relative to DST)
# ---------------------------------------------------------------------------

FILE_MAP = {
    # HTML
    "anita/index.html": "index.html",
    ...
    "wp-content/themes/uncode/library/css/style.css?ver=2089734737.css":
        "assets/css/uncode-style.css",
    "wp-content/themes/uncode/library/css/style-custom.css?ver=2089734737.css":
        "assets/css/uncode-style-custom.css",
    "wp-content/themes/uncode/library/css/uncode-icons.css?ver=2089734737.css":
    ...
    "wp-content/uploads/elementor/css/post-90612.css?ver=1774512376.css":
        "assets/css/elementor-post-90612.css",
    "wp-content/uploads/elementor/google-fonts/css/courierprime.css?ver=1742275369.css":
        "assets/css/font-courierprime.css",
    ...
    "wp-content/themes/uncode/library/js/app.js?ver=2089734737":
        "assets/js/uncode-app.js",
    "wp-content/themes/uncode/library/js/init.js?ver=2089734737":
        "assets/js/uncode-init.js",
    "wp-content/themes/uncode/library/js/plugins.js?ver=2089734737":
...
# Google font woff2 files: keep filename, move to assets/fonts/
...
IMG_MAP = {
    ...
    "wp-content/uploads/2021/09/Starling.jpg":
        "assets/img/Starling.jpg",
    "wp-content/uploads/2021/10/Duo_final_2.jpg":
        "assets/img/Duo_final_2.jpg",
    "wp-content/uploads/2021/10/explainer-1024x692.jpg":
        "assets/img/explainer-1024x692.jpg",
    "wp-content/uploads/2021/10/Hands_2-773x1024.jpg":
        "assets/img/Hands_2-773x1024.jpg",
}

# ilightbox images: flatten subdirectory prefix into filename
...
for f in ILIGHTBOX_SRC.rglob("*"):
    if f.is_file():
        ...
        # e.g. black-skin/alert.png → ilightbox-black-alert.png
        #      social_icons/facebook_16.png → ilightbox-social-facebook_16.png
        #      css/closedhand.cur → already handled above
        parts = rel.parts
        if parts[0] == "css":
        ...
        IMG_MAP[src_key] = f"assets/img/{new_name}"

FILE_MAP.update(IMG_MAP)

# ---------------------------------------------------------------------------
# Build reverse lookup: what appears in HTML/CSS → new relative path from root
# This covers the HTML references (which use %3F for ?)
# ---------------------------------------------------------------------------

def make_html_ref(old_disk_key):
    """Convert disk path to the URL form as it appears in the mirrored HTML."""
    # HTML references assets from anita/ with ../ prefix and %3F-encoded ?
    return "../" + old_disk_key.replace("?", "%3F")


def make_css_new_ref(new_dst_path, from_css_dir="assets/css"):
    """Make a relative URL from a CSS file to a destination asset."""
    # All CSS is in assets/css/, so assets/fonts/foo → ../fonts/foo
    dst = Path(new_dst_path)
    css_dir = Path(from_css_dir)
    # Compute relative path
    rel = os.path.relpath(dst, css_dir)
    return rel.replace("\\", "/")


# ---------------------------------------------------------------------------
# CSS rewrites: old url() content → new url() content
# These are applied after copying, within each CSS file.
# ---------------------------------------------------------------------------

CSS_URL_REWRITES = {
    # --- revslider.css ---
    # fonts: strip /revicons/ subdir and query string
    ...
    # cursors (were alongside the CSS file, now in img/)
    "openhand.cur": "../img/revslider-openhand.cur",
    "closedhand.cur": "../img/revslider-closedhand.cur",
    # images (were in ../assets/ relative to css/, now in img/)
    ...
    # --- elementor-icons.min.css: strip query strings from font refs ---
    ...
    # --- font-awesome CSS: webfonts/ → fonts/ ---
...
# ---------------------------------------------------------------------------
# HTML rewrites: what appears in src/href → new path (relative to index.html)
# The HTML lives at anita/index.html so all assets are referenced as ../...
# After moving to root, they become assets/...
# ---------------------------------------------------------------------------

def build_html_rewrites():
    rewrites = {}
    for old_key, new_dst in FILE_MAP.items():
        if old_key == "anita/index.html":
            continue
        # URL as it appears in the HTML (with %3F instead of ?)
...
def apply_css_url_rewrites(text):
    """Apply CSS-specific url() rewrites. Handles both quoted and unquoted forms."""
    for old, new in CSS_URL_REWRITES.items():
        # Replace in all quoting contexts
